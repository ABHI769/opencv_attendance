...
def scan_id_card():
    try:
        ...
        error_msg = str(e)
        print(f"Error scanning ID card: {error_msg}")
        if 'tesseract is not installed' in error_msg.lower() or 'not found' in error_msg.lower():
            return jsonify({
                'error': 'Tesseract OCR is not installed or not in PATH on this system. Please enter student name and roll number manually.'
            }), 400
        return jsonify({'error': f'Failed to process ID card: {error_msg}'}), 500
...
# Serve static frontend files (used when running backend locally or on Render)
@app.route('/')
def serve_frontend():
...
def serve_static_files(path):
    ...
    if '.' in path and os.path.isfile(file_path):
